...
- Here you can see this dict:
 ...
 database_setup = {
    ...
    "password" : "yourpassword", # if applicable. If no password, just type in None
...
Create new category.
```bash
curl -X POST http://127.0.0.1:5000/categories -d '{ "type" : "Economy"}' -H 'Content-Type: application/json'
```
...
```bash
curl -X POST http://127.0.0.1:5000/categories -d '{ "name" : "Economy"}' -H 'Content-Type: application/json'
```
