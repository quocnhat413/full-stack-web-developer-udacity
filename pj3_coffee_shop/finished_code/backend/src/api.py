...
def create_drink(payload):
    ...
    new_drink.insert()
    new_drink = Drink(title=body.get('title', None),
                      recipe=json.dumps(
...
def update_drink(payload, drink_id):
    ...
    if updated_recipe:
        drink_to_update.recipe = json.dumps(body.get('recipe', None))
...
def delete_drinks(payload, drink_id):
    ...
    if not drink_to_delete:
        abort(
            404, {'message': 'Drink with id {} not found in database.'.format(drink_id)})
